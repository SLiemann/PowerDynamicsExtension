...
function pg_cb_oltc() 
    buses=OrderedDict(
        "bus0" => SlackAlgebraic(U=1.0), # 1.0532 for 100% I , 1.0495 for 100% Z 1.054080675
        "bus1" => VoltageDependentLoad(P=0.0, Q=0.0, U=1.0, A=1.0, B=0.0,Y_n = complex(0.0)),
        "bus2" => VoltageDependentLoad(P=-0.9, Q=-0.5, U=1.0, A=1.0, B=0.0,Y_n = complex(0.0)))

    branches=OrderedDict(
        "Line_0-1"=> PiModelLine(from= "bus0", to = "bus1",y=1.0/(0.05+1im*0.1), y_shunt_km=0.0, y_shunt_mk=0.0),
        "OLTC"=> StaticPowerTransformerTapParam(from="bus1",to="bus2",Sbase=100e6,Srated=100e6,uk=0.10,XR_ratio=Inf,
...
function simulate_Example_OLTC(tspan::Tuple{Float64,Float64})
    ...
    function TapState(integrator)
        ...
        integrator.p[1] += 1 #position of parameter have to be selected automatically 
